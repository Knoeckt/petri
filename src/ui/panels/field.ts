...
import { siteOpen, startTrip, mgStart, mgDrop, mgPos, item, fmtDur, sitesOpen } from '../../sim';
...
export const fieldPanel: PanelDef = {
  ...
  render(el, g) {
    ...
    el.innerHTML = `<div class="scenewrap" data-scene="map"></div>` + status + `<h4 class="gh">The pipette · ${s.tickets} ticket${s.tickets === 1 ? '' : 's'}</h4>` + pip;
  ...
  act: {
    ...
    drop: (_b, g) => { const ok = mgDrop(g, mgPos(g, performance.now())); if (ok) play(g.mg?.result && g.mg.result.grade >= 2 ? 'find' : 'tap'); return ok; },
