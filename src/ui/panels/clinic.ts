...
export function reqChips(g: Ctx, list: Req[]) {
  return `<div class="needs">${list.map(q => { const h = Math.min(reqHave(g, q), q.n); const ok = h >= q.n; const col = q.t === 'med' ? null : RAR[q.t === 'any' ? 0 : q.r].col; return `<span class="chip ${ok ? 'ok' : ''}">${col ? `<i style="background:${col}"></i>` : '💊'}${reqName(g, q)} ${h}/${q.n}</span>`; }).join('')}</div>`;
}
...
export const clinicPanel: PanelDef = {
  ...
  render(el, g) {
    ...
    if (st.done) {
      ...
      const hint = step.outbreak || ok ? '' : list.some(q => q.t === 'med') ? (unlocked(g, 'brew') ? `<p class="sub" style="margin:6px 0 0">Medicines are brewed at the Apothecary from spare samples.</p>` : '')
        : list.some(q => q.t === 'rarity') ? `<p class="sub" style="margin:6px 0 0">Rarer strains show up as the dish levels. The odds bar under the dish shows your chances.</p>`
      ...
      main = `<div class="card req"><div class="who"><span class="face">${step.face}</span><div><b>${esc(step.who)}</b><div class="sub" style="margin:0">${step.outbreak ? 'needs you at the ' + TEXT.dish : 'wants ' + esc(reqSummary(g, list))} · pays ${fmt(step.reward * tierMult(g))}</div></div></div><p class="say">“${esc(say)}”</p>${step.outbreak ? '' : reqChips(g, list)}${hint}<div class="acts" style="margin-top:8px">${act}</div></div>`;
    }
    const apo = !st.done && unlocked(g, 'brew') && stepReqs(s.tier, st.step).some(q => q.t === 'med') ? `<div class="acts" style="margin:2px 0 8px"><button class="btn" data-act="open" data-tab="brew">⚗️ Brew medicines at the Apothecary${s.brew ? ' · brewing' : ''}</button></div>` : '';
  ...
  act: {
    deliver: (_b, g) => deliver(g),
    face: (_b, g, api) => { if (faceBloom(g)) api.close(); return false; },
  },
};
