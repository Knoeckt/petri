// The Catalog: every strain of this tier, found or not, with portraits drawn by the critter renderer; then hybrids.
...
export const catalogPanel: PanelDef = {
  ...
  render(el, g) {
    ...
    const groups = RAR.map((R, r) => `<div class="rgroup"><h4>${R.n} · ${def.items[r].filter((_, i) => c[`${r}-${i}`]).length}/${COUNTS[r]} · +${BONUS[r] * 100}% each</h4><div class="items">${def.items[r].map((it, i) => c[`${r}-${i}`]
      ? `<div class="it" data-r="${r}"><span class="band" style="background:${R.col}"></span>${it.danger ? `<span class="dg" title="Dangerous">⚠️</span>` : ''}<canvas class="pc" width="144" height="144" data-r="${r}" data-i="${i}"></canvas><div class="nm">${it.n}</div><div class="ds">${it.d}</div><small>${stock(g, r, i)}/${shelfCap(g)} spare · ${fmt(VAL[r] * tierMult(g) * valMult(g))} each</small></div>`
    ...
    el.innerHTML = `<p class="sub">${def.n} · ${found(g, t)}/${PER} found · catalog bonus +${Math.round((catMult(g) - 1) * 100)}% income, kept forever. Spare copies are ingredients; the shelf holds ${shelfCap(g)} of each and the rest sells.${earlier ? ' Earlier: ' + earlier : ''}</p>` + groups + hyb;
