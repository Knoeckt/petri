...
import { simulate, tierDef, COUNTS, item } from '../sim';
import type { Sheet } from './sheet';

export function openDev(g: Ctx, sheet: Sheet, app: { offline: (o: any) => void; toast: (m: string) => void }) {
  const items: [string, string][] = [['cur', '+1000 biomass'], ['notes', '+50 notes'], ['stock', '+4 of everything'], ['tix', '+3 tickets'], ['away', 'Simulate 1 h away'], ['step', 'Next story step'], ['reset', 'Reset save']];
