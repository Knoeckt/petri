...
export class Guide {
  ...
  private dim(r: DOMRect, round: boolean) {
    const base = this.root.getBoundingClientRect(); const W = base.width, H = base.height, pad = 5;
    ...
    this.spot.style.left = (x0 - 3) + 'px'; this.spot.style.top = (y0 - 3) + 'px'; this.spot.style.width = (x1 - x0) + 'px'; this.spot.style.height = (y1 - y0) + 'px'; this.spot.classList.toggle('round', round); if (this.spot.hidden) this.spot.hidden = false;
  }
  ...
  tick() {
    ...
    if (soft) { for (const b of this.blocks) if (!b.hidden) b.hidden = true; if (!this.spot.hidden) this.spot.hidden = true; } else this.dim(r, t[0] === '.vwrap');
